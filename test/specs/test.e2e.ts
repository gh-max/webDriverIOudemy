...
const user_records = parse(
  fs.readFileSync(path.join(__dirname, 'test.e2e.users.input.csv')),
  {
    columns: true,
...
const ticket_records = parse(
  fs.readFileSync(path.join(__dirname, 'test.e2e.tickets.input.csv')),
  {
    columns: true,
    skip_empty_lines: true,
  },
...
describe('new ticket creation test suite', () => {
  for (const usr of user_records) {
    it(`login user: ${usr.username}`, async () => {
      ...
      await global.log(`login user: ${usr.username}`)
